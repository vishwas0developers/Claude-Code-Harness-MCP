...
**How to run each:**
- **Terminal** (✅ in the Terminal column): install once with `npm install -g claude-code-harness-mcp`,
  then run as `claude-code-harness-mcp <command>`.
...
> 💻 **Syntax:**
> ```bash
> claude-code-harness-mcp setup <agent>
> # e.g. claude-code-harness-mcp setup antigravity
> ```
>
...
> file, a version mismatch, or a leftover pre-rename `Claude-Harness-MCP`/`Claude-Code-Harness-MCP`
> skill directory from an older install — against the currently installed package version.
> Runs with no arguments; `--check-only` below is optional.
>
> 💻 **Syntax:**
> ```bash
> claude-code-harness-mcp doctor
> # or, to only report drift without repairing it:
...
> yourself, call the `route_task` MCP tool for every functionality task, and verify each
> routed task's result before considering it done.
...
| :--- | :--- | :--- |
| `route_task` | Host agent / `/claude-code-harness-mcp start` | Main entry point — classifies a task and executes it via Claude Code if it's functionality/logic work. |
